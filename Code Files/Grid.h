...
public:
	...
	Player* GetNextPlayer(Player* p); // Gets a Pointer to the first player after the passed player
	Card* GetCard(const CellPosition& position); // Gets a Card if present in the passed CellPosition (to be used for copy/cut)
	bool isEmpty(const CellPosition& position) const; // Checks if the passed cell is empty of gameobjects (to be used in paste)
	Player* GetPoorestplayer(Player* p);
	void ResetAllPlayers();
	bool IsOverlapping(GameObject* newObj) const; //Checks if a ladder is overlapping with another ladder or snake or vice versa
	...
	void OpenAll(ifstream& InFile, ObjectType type);
