...
int Output::GetCellStartY(const CellPosition& cellPos) const
{
	...
	int y = UI.ToolBarHeight + (cellPos.VCell() * UI.CellHeight);
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\Menu_Input_Value.jpg";
...
void Output::DrawPlayer(const CellPosition& cellPos, int playerNum, color playerColor) const
{
	...
	//isValidCell
	if (playerNum < 0 || playerNum > (MaxPlayerCount - 1) || !cellPos.IsValidCell())
		return;
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if (fromCell.HCell() != toCell.HCell() || fromCell.VCell() <= toCell.VCell() || !fromCell.IsValidCell() || !toCell.IsValidCell())
		return;
	...
	//int y2 = toStartY + UI.LadderYOffset; me4 homma keda nafs el 7aga??
	...
	int count = fromCell.VCell() - toCell.VCell();
	for (int i = 0; i < count; i++) {
		pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
		int x1 = cellStartX + UI.LadderXOffset;
		int x2 = cellStartX + UI.CellWidth - UI.LadderXOffset;
		int y = fromStartY - i * UI.CellHeight;
		pWind->DrawLine(x1, y, x2, y);
	}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int horizontal[4];
	int vertical[4];
	horizontal[0] = x12 - xChange;
	vertical[0] = y1;
	horizontal[1] = x12;
	vertical[1] = y1 + yChange;
	horizontal[2] = x12 + xChange;
	vertical[2] = y1;
	horizontal[3] = x12;
	vertical[3] = y1 - yChange;

	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way

	pWind->DrawPolygon(horizontal, vertical, 4);
