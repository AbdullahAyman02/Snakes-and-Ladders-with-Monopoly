...
string Input::GetSrting(Output* pO) const
{
	...
	while (1)
	{
		...
		if (Key == 13 && Label.size() != 0)	// ENTER key is pressed
		{
			return Label;
		}
		if ((Key == 8) && (Label.size() >= 1))	// BackSpace is pressed
			Label.resize(Label.size() - 1);
		else if(!(Key == 13 && Label.size()==0))
			Label += Key;
		if (pO)
...
int Input::GetInteger(Output* pO) const
{
	...
	userInputString = GetSrting(pO);
	int userInputInt = stoi(userInputString);


	// Note: stoi(s) converts string s into its equivalent integer (for example, "55" is converted to 55)

	return userInputInt; // this line should be changed with your implementation
}
...
ActionType Input::GetUserAction() const
{
	...
	else
	{
		...
		if (y >= 0 && y < UI.ToolBarHeight)
		{
			...
			case ITM_INPUT_DICE_VALUE: return INPUT_DICE_VAUE;
			case ITM_NEW_GAME: return NEW_GAME;
			case ITM_SWITCH_TO_DESIGN_MODE: return TO_DESIGN_MODE;  // just for now ==> This should be updated
...
CellPosition Input::GetCellClicked() const
{
	...
	if (UI.InterfaceMode == MODE_DESIGN || UI.InterfaceMode == MODE_PLAY)
	{
		if (y >= UI.ToolBarHeight && y <= (UI.height - UI.StatusBarHeight))
		{
			...
			int newY = y - UI.ToolBarHeight;
			if (newY % UI.CellHeight != 0)
			{
				cellPos.SetVCell((newY / UI.CellHeight));
			}
			else
			{
				cellPos.SetVCell((newY / UI.CellHeight) - 1);
			}
