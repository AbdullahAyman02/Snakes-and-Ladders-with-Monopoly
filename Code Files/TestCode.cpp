...
int main()
{
	...
	string message = to_string(pIn->GetInteger(pOut));
	pOut->PrintMessage("You entered: " + message + " ------> Click anywhere to continue.");
	...
	pOut->PrintMessage("Click on a cell to start the test");
	for (int i = 0; i < 5; i++)
	{
		CellPosition pos = pIn->GetCellClicked();
		string message1 = to_string(pos.VCell());
		string message2 = to_string(pos.HCell());
		pOut->PrintMessage(message1 + " " + message2 + " ------> Click on the next cell.");
	...
	for (int i = 0; i < 5; i++) {
		pOut->PrintMessage("Enter a vCell");
		int a = pIn->GetInteger(pOut);
		cellpos_1.SetVCell(a);
		string message1 = "Now the vCell = " + to_string(cellpos_1.VCell());
		pOut->PrintMessage(message1 + " ------> Click anywhere to continue.");
		pIn->GetPointClicked(x, y);
	}
	...
	for (int i = 0; i < 5; i++) {
		...
		cellpos_1.SetHCell(b);
		string message1 = "Now the hCell = " + to_string(cellpos_1.HCell());
		pOut->PrintMessage(message1 + " ------> Click anywhere to continue.");
		pIn->GetPointClicked(x, y);
	}
	...
	for (int i = 0; i < 5; i++) {
		...
		int x1 = pIn->GetInteger(pOut);
		pOut->PrintMessage("Enter hCell");
		int x2 = pIn->GetInteger(pOut);
		CellPosition cellp(x1, x2);
		int x3 = CellPosition::GetCellNumFromPosition(cellp);
		pOut->PrintMessage("CellNum = " + to_string(x3) + " ------> Click anywhere to continue.");
		pIn->GetPointClicked(x, y);
	}
	...
	for (int i = 0; i < 5; i++) {
		...
		int x5 = pIn->GetInteger(pOut);
		CellPosition pcell(x5);
		pOut->PrintMessage(to_string(pcell.VCell()) + ", " + to_string(pcell.HCell()) + " ------> Click anywhere to continue.");
		pIn->GetPointClicked(x, y);
	}
	...
	///TODO:  
	//You must add a case for EACH action (both Create-Gride mode and Game mode actions)
	...
	} while (ActType != EXIT && ActType != EXITP);
